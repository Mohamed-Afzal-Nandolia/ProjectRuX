...
export function OTPInput({
  ...
  const [internalDigits, setInternalDigits] = React.useState<string[]>(Array.from({ length }, () => ""))
  const inputsRef = React.useRef<Array<HTMLInputElement | null>>([])

  const digits = isControlled ? Array.from({ length }, (_, i) => value?.[i] ?? "") : internalDigits
  ...
  return (
    <div className={cn("flex flex-col items-center gap-3", className)}>
      ...
      {name ? <input type="hidden" name={name} value={digits.join("")} /> : null}

      <p className="sr-only" aria-live="polite">
        {digits.every((d) => d !== "") ? "Code complete" : "Enter your verification code"}
      </p>
    </div>
  )
}
